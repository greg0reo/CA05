...
//	cout << randDeck[2].n << randDeck[2].f << endl;
	...
	while(one.deck.size() != 0 && two.deck.size() != 0){
		...
		if(one.deck.front() == two.deck.front()){
			...
			if(one.deck.front() < two.deck.front()){
				...
				cout << two.getName() << " wins the war with a " << two.deck.front().reader() << " against " << one.getName() << "'s " << one.deck.front().reader() << endl;
				logThis += two.getName();
				logThis += "wins the war with a ";
				logThis += two.deck.front().reader();
				logThis += " against ";
				logThis += one.deck.front().reader();
				logThis += "\n";
			...
			// ONE WINS WAR
				...
				cout << one.getName() << " wins the war with a " << one.deck.front().reader() << " against " << two.getName() << "'s " << two.deck.front().reader() << endl;
				logThis += one.getName();
				logThis += "wins the war with a ";
				logThis += two.deck.front().reader();
				logThis += " against ";
		...
		// TWO WINS
			...
			cout << two.getName() << " wins the draw" << endl;
			logThis += two.getName();
			logThis += " wins the draw\n";
		...
		// ONE WINS
			...
			cout << one.getName() << " wins the draw" << endl;
			logThis += one.getName();
			logThis += " wins the draw\n";
	...
	if(one.deck.size()==0){
		...
		logger << logThis;
		logger.close();
//		logger.write((char*)&logThis, sizeof(logThis));
